...
%  title('Board')
%
%  See also insertShape, insertMarker, insertObjectAnnotation, showShape,
%           listTrueTypeFonts.
...
crossCheckFontFontSize(font, fontSize);
...
if isnumeric(text)
    % No objects allowed.
    vision.internal.inputValidation.validateNotObject(text, 'vision', 'text');
...
%==========================================================================
function crossCheckFontFontSize(fontName, fontSize)
isMSMinchoGT15 = strcmpi(fontName, 'MS Mincho') &&  (fontSize > 15);
errIf0(isMSMinchoGT15, 'vision:insertText:MSMinchoOver15');
...
% No objects allowed.
vision.internal.inputValidation.validateNotObject(opacity, 'vision', 'BoxOpacity');
...
% No objects allowed.
vision.internal.inputValidation.validateNotObject(ShapeWidth, 'vision', 'ShapeWidth');
...
% No objects allowed.
vision.internal.inputValidation.validateNotObject(ShapeHeight, 'vision', 'ShapeHeight');
...
if isSimMode
    ...
    % use dummyOut to model the fact that sprintf returns an int
    dummyOut = int32(0);
    dummyOut = coder.ceval('sprintf', coder.ref(str1), cstring('%0.5g'), numDbl); %#ok<NASGU>
    str = mstring(str1);
end
