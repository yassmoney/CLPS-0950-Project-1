...
allColors1 = [.2 .2 .1 .3 ; .6 .1 .1 .8;  1 .4 .8 .9]%Set the RBG values of each rectangle; The order is as follows: [R R R R; G G G G; B B B B]; the order is not [1,2,3,4,] but instead is [1, 4, 3, 2]; This allows for the correct order on screen during the test. 
...
% Set the colors to SKY BLUE, WATER BLUE, DARK BLUE, ROYAL BLUE 
allColors = [ .1 .2 .3 .2 ; .1 .1 .8 .6;  .8 .4 .9 1 ]%Set the RBG values of each rectangle; The order is as follows: [R R R R; G G G G; B B B B]; the order is not [1,2,3,4,] but instead is [4, 3, 2, 1]; This allows for the correct order on screen during the test. 
...
%----------------------------------------------------------------------
 ...
 DrawFormattedText(window, char(theWord), 'center', 'center', theColor);
        ...
        if keyCode(escapeKey)
            ...
            sca;
            return
        elseif keyCode(skyblueKey)%S key corrosponds to color 1 (sky) 
            response = 1;
            respToBeMade = false;
        elseif keyCode(waterblueKey)%W key corrosponds to color 2 (water) 
            response = 2;
            respToBeMade = false;
        elseif keyCode(darkblueKey)%D key corrosponds to color 3 (dark blue)
            response = 3;
            respToBeMade = false;
        elseif keyCode(royalblueKey)%R key corrosponds to color 4 (royal)
            response = 4;
            respToBeMade = false;
        end
...
sca; %clear screen to prep for next level 